...
{
    CyRamVectors[CYINT_IRQ_BASE + UART_RX_INTERRUPT__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + UART_RX_INTERRUPT__INTC_NUMBER];
}
...
void UART_RX_INTERRUPT_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((UART_RX_INTERRUPT__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *UART_RX_INTERRUPT_INTC_PRIOR = (*UART_RX_INTERRUPT_INTC_PRIOR & (uint32)(~UART_RX_INTERRUPT__INTC_PRIOR_MASK)) |
...
uint8 UART_RX_INTERRUPT_GetPriority(void)
{
    ...
    priority = (*UART_RX_INTERRUPT_INTC_PRIOR & UART_RX_INTERRUPT__INTC_PRIOR_MASK) >> priorityOffset;
