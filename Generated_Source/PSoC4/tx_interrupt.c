...
{
    CyRamVectors[CYINT_IRQ_BASE + tx_interrupt__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + tx_interrupt__INTC_NUMBER];
}
...
void tx_interrupt_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((tx_interrupt__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *tx_interrupt_INTC_PRIOR = (*tx_interrupt_INTC_PRIOR & (uint32)(~tx_interrupt__INTC_PRIOR_MASK)) |
...
uint8 tx_interrupt_GetPriority(void)
{
    ...
    priority = (*tx_interrupt_INTC_PRIOR & tx_interrupt__INTC_PRIOR_MASK) >> priorityOffset;
